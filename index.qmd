...
Nous parlons de régression linéaire **multiple** si il y a plus d'une
...
linéaire et vérifier si les hypothèses du modèle sont satisfaites. En
utilisant la fonction `plot()` après avoir calculé un modèle de
régression linéaire avec `lm()` dans R, nous pouvons visualiser quatre
types de plots diagnostiques :

1.  **Résidus vs valeurs prédites** : Ce plot montre la relation entre
    les résidus (la différence entre les valeurs réelles et les valeurs
    prédites) et les valeurs prédites. Si les points sont aléatoires
    autour de la ligne horizontale à zéro, cela indique que les résidus
    sont indépendants et que la variance est constante.
2.  **Résidus vs variables indépendantes** : Ce plot montre la relation
    entre les résidus et chaque variable indépendante. Si les points
    sont aléatoires autour de la ligne horizontale à zéro, cela indique
    que la relation entre les variables indépendantes et la variable
    dépendante est linéaire.
3.  **QQ-plot** : Ce plot compare les quantiles des résidus standardisés
    avec les quantiles d'une distribution normale standard. Si les
    points sont proches de la ligne diagonale, cela indique que les
    résidus suivent une distribution normale.
4.  **Densité des résidus** : Ce plot montre la densité des résidus. Si
    la densité est symétrique autour de zéro et suit une forme de
    cloche, cela indique que les résidus suivent une distribution
    normale.

En examinant cesgraphiques de diagnostic, nous pouvons évaluer si les
...
normalité des résidus. Si les hypothèses ne sont pas satisfaites, cela
peut indiquer que le modèle n'est pas adéquat et qu'une transformation
des données ou une sélection de variables différente peut être
nécessaire.
...
Dans la pratique, **il suffit d'évaluer visuellement les graphiques de
diagnostic**. Si les lignes rouges qui indiquent les valeurs moyennes
des résidus (les points) ne s'écartent pas de manière importante des
lignes pointillées, le modèle de régression est bon à utiliser. Si les
écarts sont importants, il peut être nécessaire de trouver un autre
modèle ou de transformer les données (par exemple, transformation
logarithmique, etc.). Dans le cas spécifique, les graphique de
diagnostic sont acceptable.
...
essentiel de vérifier les graphiques des résidus pour valider les
hypothèses d'homoscédasticité, de normalité et d'indépendance des
résidus :
...
-   Attention, **la session 14 du 29 et 30 mai 2024 sera une session en
    auto-apprentissage !** Il n'y a donc pas d'enseignement en classe à
    Fribourg.La session 14 vous fournira surtout des ressources R plus avancées que vous pourrez utiliser plus tard, par exemple pendant votre travail de bachelor, pour vos analyses empiriques. 