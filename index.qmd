...
participé au Problem Set 1. Le nombre maximal de points était de 20, il
...
une ligne de régréssion linéaire avec `geom_smooth(method = lm)` :
...
Que nous indique la ligne de régréssion ? En principe, elle indique une
...
données `diamonds`. Cette-fois ci, ous sauvegardons et nommons le
graphique `db1`.
...
print(db1)
```
Que nous dit le graphique ? Il est intéressant de constater que dans le jeu de données `diamonds`, le niveau de qualité le plus élevé de la taille, à savoir `idéal`, est également le plus fréquent.

::: {.callout-caution collapse="false"}
## Attention

Attention ! Dans la fonction `ggplot()`, il y a une différence si vous
...
égale." [Statistique
Canada](https://www150.statcan.gc.ca/n1/edu/power-pouvoir/ch9/histo/5214822-fra.htm)
...
distribution*). Utilisez aussi `?rnorm()`pour accéder à plus d'infos.
...
Selon un article de [*24heures*](https://www.24heures.ch/calculateur-en-ligne-quelle-taille-faites-vous-par-rapport-aux-autres-191558705635#), les femmes ont une taille moyenne de 164.7 cm tandis que les hommes mesurent 177.4 cm en moyenne. Dans la suite, nous supposons que les écarts-types sont de +- 5.6 cm pour les femmes et de +- 6.1 cm pour les hommes.
...
-   **Annonce : Questionnaire**
    ...
    -   Chaque étudiant(e) qui aura rempli le questionnaire dans les
        délais recevra un **bonus de 3 points dans le Problem Set 4**
        (10% des points).
