...
3.  **Indépendance**
    ...
    -   Pour les tests t à deux échantillons, les échantillons doivent
        ...
        un test t par paires peut être approprié.
4.  **Normalité**
...
2.  **Le test t à deux échantillons indépendants** (angl. two sample
    t-test ; aussi appelé Welch two sample t-test) : pour évaluer si les
    moyennes de deux échantillons indépendants diffèrent de manière
    significative.
...
-   Notre hypothèse H1 stipule qu'il y a une différence significative
    entre le QI des étudiant-e-s et celui de la population générale. En
    effet, nous assumons que le QI des étudiant-e-s est plus élevé que
    celui de la population.

-   L'hypothèse nulle H0 serait donc le contraire, c'est à dire que la
    différence n'est pas significative entre les deux groupes.
...
# Il faut 2 arguments dans la fonction t.test() : les données à tester (scores_qi) et la valeur à laquelle on veut comparer les données (mu = 100)
