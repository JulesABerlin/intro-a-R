...
[![Volets
RStudio](docs/assets/volets_rstudio.png)](https://docs.posit.co/ide/user/ide/guide/ui/ui-panes.html)
...
::: {.callout-important title="Important"}
-   Il est de convention qu'un tableau de données présente les
    **observations** (ou les cas) **par ligne** (angl. *row*).
-   Les **variables** ou caractéristiques de ces observations sont
    indiquées en conséquence **par colonnes** (angl. *column*).
:::
...
Les types de données suivants en sciences de la communication se
présentent généralement sous forme de tableaux :

-   les **réponses à des questionnaires** (où chaque ligne correspond à
    un répondant)

-   les **données relatives au contenu des médias** (où chaque ligne
    correspond à un document, par exemple un article de presse)

-   les **données relatives aux médias sociaux** (où chaque ligne
    correspond à un profil ou à un post sur les médias sociaux)
:::

Voici un modèle de la manière dont on devrait structurer les réponses à
un questionnaire avec n répondants et X questions :
...
A noter : Si vous ne spécifier pas tous les arguments, R va utiliser des
