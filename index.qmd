...
Les résultats du **Problem Set 1** sont accéssibles sur la page Moodle de l'exercices.
...
`ggplot2` est un paquet de visualisation de données populaire dans R,
conçu pour faciliter la création de graphiques complexes et multicouches
avec une syntaxe cohérente et consistante.

Faisant partie du `tidyverse`, `ggplot2` est basé sur la syntaxe
*Grammar of Graphics*, un cadre pour la description et la construction
de graphiques.
...
Pour comprendre exactement comment cela fonctionne, voici un exemple de code dans lequel les différentes couches d'un graphique ggplot sont ajoutées étape par étape. (Normalement, nous ne procéderions pas ainsi, mais utiliserions directement le code pour le graphique fini). 

Le but dans cet exemple est de visualiser le jeu de données `diamonds`pour comprendre la relation entre les variables prix et carat (poids) des diamants !

```{r}
# La première couche, data, définit le jeu de données
ggplot(data = diamonds)
```
Notre graphique n'est pour l'instant qu'une zone grise ! Catastrophe ? Bien sûr que non 😀. Nous devons maintenant définir les autres couches de ggplot !
...
Maintenant il s'agit d'interpréter le graphique. Que nous dit-il sur la relation entre prix et carats des diamants ? Quel rôle joue la variable *clarity* ?
