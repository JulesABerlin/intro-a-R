...
**Sujet : La statistique inférentielle**
...
Avec un tel résultat de test t, **nous pouvons rejetter l'hypothèse
nulle H0**, car la probabilité que les valeurs observées soient le
résultat d'un pure hasard sont très faible (moins de 1%). Nous pouvont
alors inférer que le QI des étudiant(e)s est significativement plus
...
La très petite valeur p indique qu'il est extrêmement improbable que
l'utilisation d'Internet des 20-29 ans ne diffère PAS significativement
de la moyenne nationale. Nous pouvons alor rejetter H0. En d'autres
termes : Nous pouvons supposer avec une très grande certitude que les
20-29 ans ont une utilisation d'Internet significativement plus élevée
...
improbable que 24 hommes suisses tiré au hasard sont envirion de même
taille que 24 joueuses de la WNBA. Nous pouvons alors rejetter H0. En
...
## L'analyse de la variance (ANOVA) (sous construction)

L'ANOVA est utilisée pour comparer les moyennes de plus de deux groupes.
Elle permet de déterminer s'il existe une différence significative entre
les groupes. L'ANOVA est, de ce point de vue, une généralisation du test
t à plus de deux groupes qu'on veut comparer.

## La régression linéaire avec `lm()`(sous construction)

La fonction `lm()` est utilisée pour effectuer une régression linéaire.
Elle permet de modéliser la relation entre une variable dépendante et
